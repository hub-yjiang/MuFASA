...
#added by Linda to avoid the comparison of partial chromsomes from the reference

if (!is.null(refgenome)) {
    vcf_levels <- seqlevels(raw_vcf)
    ref_levels <- seqlevels(refgenome)
    common_levels <- intersect(ref_levels, vcf_levels)

    # Prune only the VCF to match the reference
    raw_vcf <- keepSeqlevels(raw_vcf, common_levels, pruning.mode="coarse")

    dropped_vcf <- setdiff(vcf_levels, common_levels)
    dropped_ref <- setdiff(ref_levels, common_levels)

    if (length(dropped_vcf) > 0) {
        write(paste("Dropped from VCF:", paste(dropped_vcf, collapse=",")), stderr())
    }
    if (length(dropped_ref) > 0) {
        write(paste("Dropped from reference (informational):", paste(dropped_ref, collapse=",")), stderr())
    }
}
